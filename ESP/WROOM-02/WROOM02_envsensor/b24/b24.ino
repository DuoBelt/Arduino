...
void loop() {
  ...
  String info = "t=" + String(t) + "&h=" +  String(h) + "&p=" + String(p) + "&mac=" + thisMAC + "&up=" + upCount++ + "&spv=" + String(loadvoltage, 3) + "&spa=" + String(current_mA, 3);
  String url = String("/tph.php?")  + info;

  url.toCharArray(buffer, sizeof(buffer));

  uploadThis(host, buffer, 80);
