...
static unsigned int pushCounter = 0; // カウンター(デバッグの際に必要なら使いましょう) // by Sekitakovich 2016-02-02

void onSwitched(){
  pushCounter++; // 上記カウンターをインクリメントするだけです // by Sekitakovich 2016-02-02
}

void setup(){
  pinMode(TACTILE_SW_PIN,INPUT_PULLUP);
// 以下2行追加 // by Sekitakovich 2016-02-02
...
// 以上2行追加 // by Sekitakovich 2016-02-02
...
void loop(){
  ...
  sleep_mode(); // ここで予め設定されたモードでsleep状態に遷移、外部割り込み(今回は1)で抜けてきます // by Sekitakovich 2016-02-02
  
  int sw = digitalRead(TACTILE_SW_PIN);
//  if (sw == LOW){ // このif文は不要です // by Sekitakovich 2016-02-02
...
//    delay(500); // このdelayは不要です // by Sekitakovich 2016-02-02

//  } // by Sekitakovich 2016-02-02
}
