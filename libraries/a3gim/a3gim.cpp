...
#  define DEBUGP(m)         Serial.print((m))
#else
...
#endif
SoftwareSerial a3gSerial(IEM_RXD_PIN, IEM_TXD_PIN);		// Serial for IEM Interface
...
//--
	if (duration == 0)
		...
		sprintf(gWorkBuffer, "$YA %d", duration);
...
{
	// Force to listen a3gSerial at multiple SoftwareSerial using
	a3gSerial.listen();
